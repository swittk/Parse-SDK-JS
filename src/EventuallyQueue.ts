...
const EventuallyQueue = {
  ...
  process: {
    create(ObjectType: typeof ParseObject, queueObject: QueueObject) {
      const object = new ObjectType();
      return EventuallyQueue.sendQueueCallback(object, queueObject);
    },
    async byId(ObjectType: string | ParseObject | (typeof ParseObject), queueObject: QueueObject) {
    ...
    async byHash(ObjectType: typeof ParseObject, queueObject: QueueObject) {
      const { sessionToken } = queueObject.serverOptions;
      const query = new ParseQuery(ObjectType);
...
export type EventuallyQueueType = {
  /**
   * Add object to queue with save operation.
   *
   * @function save
   ...
   * @ignore
   */
  enqueue(action: string, object: ParseObject, serverOptions?: RequestOptions | SaveOptions): Promise<any>;
  ...
  process: {
    create(ObjectType: typeof ParseObject, queueObject: QueueObject): Promise<void>;
    byId(ObjectType: string | ParseObject | (typeof ParseObject), queueObject: QueueObject): Promise<void>;
    byHash(ObjectType: typeof ParseObject, queueObject: QueueObject): Promise<void>;
  }
}
